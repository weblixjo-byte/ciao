...
export async function POST(req: Request) {
  try {
    ...
    await dbService.createNotification({
      ...
      message: `You earned ${pointsEarned} points on your bill of ${bill.toFixed(3)} ${config.currency} at ${session.branchName || config.storeName || "ciao ciao"}. New balance: ${newBalance} pts.`,
      type: "POINTS_EARNED",
    });
