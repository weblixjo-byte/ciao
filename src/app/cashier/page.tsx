...
export default function CashierPage() {
  ...
  const CASHIER_CACHE_KEY = "ciao_cashier_cached";
  const CASHIER_TOKEN_KEY = "ciao_cashier_token";
  ...
  const performLookup = async (query: string) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setClaimedReward(data.pendingReward || null);
      } else {
  ...
  // Handle tactile on-screen keypad press (up to 8 digits: 6 PIN + 2 Reward)
  ...
  // Redeem Locked Claimed Reward (Instant & Locked)
  const handleRedeemClaimedReward = async () => {
    if (!identifiedCustomer || !claimedReward) return;

    if (claimedReward.pointsRequired > identifiedCustomer.pointsBalance) {
      setRedeemError(`Insufficient customer balance (${identifiedCustomer.pointsBalance} pts available, ${claimedReward.pointsRequired} pts required)`);
      return;
    }
    ...
    try {
      const res = await fetch("/api/pos/redeem", {
        ...
        body: JSON.stringify({
          customerId: identifiedCustomer.id,
          pointsToRedeem: claimedReward.pointsRequired,
  ...
  if (loadingSession) {
    return (
      <div className="min-h-screen bg-[#FAFBFA] flex items-center justify-center">
        <div className="w-8 h-8 rounded-full border-2 border-neutral-300 border-t-[#36543D] animate-spin" />
      </div>
    );
  }
  ...
  if (!cashier) {
    return (
      <div className="min-h-screen bg-[#FAFBFA] flex flex-col justify-between p-4 sm:p-6 select-none font-sans">
        <div className="max-w-sm w-full mx-auto my-auto py-4">
          <div className="text-center mb-6">
            <div className="w-16 h-16 rounded-2xl bg-white border border-neutral-200 shadow-xs p-1 object-contain flex items-center justify-center mx-auto mb-3 overflow-hidden">
            ...
            <p className="text-xs text-neutral-500 font-medium font-sans">
              Point of Sale & Loyalty • {config.storeName || "ciao ciao"} POS
            </p>
          </div>
          ...
          <div className="glass-panel rounded-3xl p-6 sm:p-7 shadow-xl">
            ...
            <form onSubmit={handleCashierLogin} className="space-y-4">
              ...
              <button
                ...
                className="w-full py-3.5 rounded-xl bg-[#36543D] text-white text-sm font-bold hover:bg-[#2a4230] transition-all disabled:opacity-50 mt-2 flex items-center justify-center gap-2 cursor-pointer shadow-sm active:scale-98 font-sans"
              >
          ...
          {!isStandaloneApp && (
            <div className="mt-4 p-3.5 rounded-2xl glass-panel-subtle border border-neutral-200 shadow-2xs flex items-center justify-between gap-3">
              <div className="flex items-center gap-2.5 min-w-0">
                <div className="w-8 h-8 rounded-xl bg-emerald-50 text-[#36543D] flex items-center justify-center shrink-0">
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-xl bg-[#36543D] text-white text-xs font-bold shrink-0 hover:bg-[#2a4230] transition-all cursor-pointer shadow-2xs font-sans"
              >
                Install
              </button>
            </div>
            ...
            <div className="glass-panel rounded-3xl p-6 max-w-sm w-full shadow-2xl relative animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="space-y-3 glass-panel-subtle rounded-2xl p-4 text-xs text-neutral-800 font-sans">
                <div className="flex items-start gap-2.5">
                  ...
                  <span>Tap the <strong>Share</strong> button <Share2 className="w-3.5 h-3.5 inline mx-0.5 text-[#36543D]" /> in Safari or <strong>Menu (⋮)</strong> in Chrome.</span>
                </div>
                <div className="flex items-start gap-2.5">
                ...
                <div className="flex items-start gap-2.5">
                  ...
                  <span>Tap <strong>Add</strong>. The icon will be named <strong>{config.storeName || "ciao ciao"} Cashier</strong> and will open this Cashier POS terminal directly.</span>
                </div>
              </div>
              ...
              <button
                type="button"
                onClick={() => setShowInstallGuide(false)}
                className="w-full mt-4 py-2.5 rounded-xl bg-[#36543D] text-white text-xs font-bold hover:bg-[#2a4230] transition-colors cursor-pointer font-sans"
              >
                Got It
              </button>
            </div>
        ...
        <div className="text-center text-xs text-neutral-500 py-4 flex items-center justify-center gap-4">
          <Link href="/admin" className="hover:text-[#36543D] font-semibold transition-colors">
            Admin Console
          </Link>
          <span className="text-neutral-300">•</span>
          <Link href="/customer" className="hover:text-[#36543D] font-semibold transition-colors">
            Customer Pass
          </Link>
        </div>
      </div>
  ...
  // ==========================================
  return (
    <div className="min-h-screen bg-[#FAFBFA]/80 backdrop-blur-md flex flex-col justify-between select-none font-sans">
      ...
      <header className="glass-panel border-x-0 border-t-0 rounded-none px-3.5 sm:px-6 py-2.5 sm:py-3 sticky top-0 z-20 shadow-xs">
        <div className="max-w-2xl mx-auto flex items-center justify-between gap-2">
          {/* Cashier Info */}
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-white border border-neutral-200 shadow-xs p-1 object-contain flex items-center justify-center shrink-0 overflow-hidden">
              <img src="/logo.png" alt={config.storeName || "ciao ciao"} className="w-full h-full object-contain" />
            </div>
            <div className="min-w-0">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            {!isStandaloneApp && (
              <button
                type="button"
                onClick={handleInstallClick}
                className="px-2.5 sm:px-3 py-1.5 rounded-xl border border-neutral-200 glass-panel-subtle hover:bg-white text-xs font-semibold text-[#36543D] flex items-center gap-1.5 transition-all cursor-pointer shadow-2xs font-sans"
            ...
            >
              <RefreshCw className="w-3.5 h-3.5 text-[#36543D]" />
      ...
      <main className="max-w-2xl mx-auto w-full px-3.5 sm:px-6 py-4 sm:py-6 flex-1 flex flex-col justify-start">
        ...
        {!identifiedCustomer && (
          <div className="w-full max-w-md mx-auto space-y-3.5">
            ...
            <div className="grid grid-cols-2 gap-2 glass-panel-subtle p-1 rounded-2xl border border-neutral-200">
              <button
                ...
                className={`py-2.5 px-3 rounded-xl font-bold text-xs flex items-center justify-center gap-2 transition-all cursor-pointer font-sans ${
                  activeMode === "pin"
                    ? "bg-[#36543D] text-white shadow-xs"
            ...
            {/* MODE 1: 6-DIGIT PIN + 2-DIGIT REWARD NUMPAD */}
            {activeMode === "pin" && (
              <div className="glass-panel rounded-3xl p-4 sm:p-5 shadow-lg space-y-4">
                ...
                {/* Action Buttons Depending on Input State */}
                {pinQuery.length >= 6 && pinQuery.length < 8 && (
                  <button
                    type="button"
                    onClick={() => performLookup(pinQuery.slice(0, 6))}
                    disabled={lookupLoading}
                    className="w-full py-3.5 rounded-2xl bg-[#36543D] text-white text-sm font-bold hover:bg-[#2a4230] transition-all disabled:opacity-40 cursor-pointer shadow-xs flex items-center justify-center gap-2 active:scale-98 font-sans"
                  >
                    {lookupLoading ? (
                      <>
                ...
                {pinQuery.length === 8 && (
                  ...
                  >
                    {lookupLoading ? (
                      ...
                      <>
                        <span>Search & Redeem Reward</span>
                ...
                {pinQuery.length < 6 && (
                  <div className="text-center text-[11px] text-neutral-400 font-sans">
                    Enter 6-digit PIN to credit points, or complete with 2 digits to redeem a reward.
                  </div>
                )}
              </div>
            )}
            ...
            {activeMode === "qr" && (
              <div className="glass-panel rounded-3xl p-5 sm:p-6 shadow-lg space-y-4 text-center">
                <div className="w-16 h-16 rounded-full bg-emerald-50 border border-emerald-100 text-[#36543D] flex items-center justify-center mx-auto">
                ...
                <button
                  ...
                  className="w-full py-4 px-4 rounded-2xl bg-[#36543D] hover:bg-[#2a4230] text-white text-sm font-bold flex items-center justify-center gap-2.5 transition-all shadow-md active:scale-98 cursor-pointer font-sans"
                >
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-4 py-2.5 rounded-xl bg-[#36543D] text-white text-xs font-bold hover:bg-[#2a4230] transition-colors disabled:opacity-40 cursor-pointer shrink-0 font-sans"
                  >
        ...
        {identifiedCustomer && !receipt && (
          <div className="w-full max-w-lg mx-auto space-y-4">
            ...
            <div className="glass-panel rounded-3xl p-4 sm:p-5 shadow-lg">
              <div className="flex items-center justify-between gap-3 mb-3">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-12 h-12 rounded-2xl bg-emerald-50 border border-emerald-100 flex items-center justify-center text-[#36543D] text-xl font-bold shrink-0">
              ...
              <div className="glass-panel-subtle rounded-2xl p-3 flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-baseline gap-1">
                    <span className="text-2xl font-black text-[#36543D] font-sans">
                ...
                <div className="text-right bg-white/80 px-3 py-1.5 rounded-xl border border-neutral-200 shadow-2xs">
                  <span className="text-[10px] text-neutral-400 block font-medium font-sans">Member Tier</span>
                  <span className="text-xs font-bold text-[#36543D] uppercase tracking-wider font-sans">
            ...
            {/* MODE A: CLAIMED REWARD REDEMPTION (Locked, No Switcher, No Dropdown, No Point Editing) */}
            {claimedReward && (
              <div className="glass-panel rounded-3xl p-5 sm:p-6 shadow-lg space-y-4">
                <div className="flex items-center justify-between pb-3 border-b border-neutral-200/60">
                  <div>
                    <span className="text-[10px] uppercase font-bold tracking-wider text-[#36543D] bg-emerald-50 px-2.5 py-0.5 rounded-md inline-block mb-1">
                ...
                {/* Single Locked Reward Presentation Card */}
                <div className="glass-panel-subtle rounded-2xl overflow-hidden border border-neutral-200">
                  {claimedReward.imageUrl && (
                    <div className="w-full h-36 overflow-hidden border-b border-neutral-200 bg-[#FAFBFA]">
                      <img
                  ...
                  <div className="p-4">
                    <div className="flex items-center justify-between gap-2 mb-1">
                      <span className="text-[10px] font-sans font-bold uppercase tracking-wide text-neutral-500">
                        {claimedReward.category || "Item Reward"}
                      </span>
                      <span className="px-2.5 py-0.5 rounded-md bg-[#36543D] text-[#F4EECF] font-mono text-[11px] font-bold shadow-2xs">
                        CODE: {claimedReward.claimCode || "10"}
                      </span>
                    </div>
                    ...
                    {/* Locked Cost Tag */}
                    <div className="p-3 rounded-xl bg-neutral-100/70 border border-neutral-200 flex items-center justify-between mt-2">
                      <span className="text-xs font-semibold text-neutral-600 font-sans">
                        Locked Redemption Cost:
                      </span>
                      <span className="text-base font-black font-mono text-[#36543D]">
                ...
                <div className="glass-panel-subtle rounded-2xl p-4 space-y-2 border border-neutral-200">
                  ...
                  <div className="pt-2 border-t border-neutral-200 flex justify-between items-center text-xs font-bold font-sans">
                ...
                {identifiedCustomer.pointsBalance < claimedReward.pointsRequired ? (
                  <div className="p-3 rounded-xl bg-red-50 border border-red-200 text-xs text-red-700 flex items-center gap-2">
                    ...
                    <span className="font-semibold">
                      Insufficient points! Customer needs {claimedReward.pointsRequired} pts, but only has {identifiedCustomer.pointsBalance} pts.
                    </span>
                  </div>
                ) : (
                  <div className="p-2.5 rounded-xl bg-emerald-50 border border-emerald-200 text-xs text-[#36543D] flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 shrink-0 text-emerald-600" />
                    <span className="font-semibold">
                      Customer is eligible to claim this reward.
                    </span>
                  </div>
                )}
                ...
                <div className="flex gap-2.5 pt-1">
                  <button
                    type="button"
                    onClick={resetPOS}
                    className="px-4 py-3.5 rounded-2xl border border-neutral-200 text-neutral-600 hover:bg-neutral-50 text-xs font-bold transition-colors cursor-pointer font-sans"
                  >
                    Cancel
                  </button>
                  ...
                  <button
                    ...
                    className="flex-1 py-3.5 rounded-2xl bg-[#36543D] hover:bg-[#2a4230] text-white text-xs sm:text-sm font-bold transition-all disabled:opacity-40 cursor-pointer shadow-sm active:scale-98 flex items-center justify-center gap-2 font-sans"
                  >
                    {redeemLoading ? (
                      <>
                        <div className="w-4 h-4 rounded-full border-2 border-white/20 border-t-white animate-spin" />
                      ...
                      <>
                        <span>
                          Confirm & Redeem {claimedReward.title} ({claimedReward.pointsRequired} pts)
                        </span>
            ...
            {/* MODE B: CREDIT POINTS ON BILL (No Switcher, No Rewards) */}
            {!claimedReward && (
              <div className="glass-panel rounded-3xl p-5 sm:p-6 shadow-lg space-y-4">
                <div className="flex items-center justify-between pb-3 border-b border-neutral-200/60">
                  <div>
                ...
                <form onSubmit={handleCreditPoints} className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="relative flex items-center">
                      <input
                        ...
                        required
                      />
                      <span className="absolute right-3 px-2.5 py-1 rounded-xl bg-neutral-100 border border-neutral-200 text-xs font-bold text-[#36543D] font-sans pointer-events-none">
                        {config.currency}
                      </span>
                    </div>
                  ...
                  <div className="p-3.5 glass-panel-subtle rounded-2xl flex items-center justify-between">
                    <div className="flex items-center gap-1.5 text-xs font-bold text-neutral-900 font-sans">
                      <Sparkles className="w-4 h-4 text-[#36543D]" />
                      <span>Points to be Earned:</span>
                    </div>
                    <span className="text-base font-black font-sans text-[#36543D]">
                  ...
                  <div className="flex gap-2.5 pt-1">
                    ...
                    <button
                      ...
                      className="flex-1 py-3.5 rounded-2xl bg-[#36543D] hover:bg-[#2a4230] text-white text-sm font-bold transition-all disabled:opacity-40 flex items-center justify-center gap-2 shadow-sm cursor-pointer active:scale-98 font-sans"
                    >
                      {transactLoading ? (
                        <>
                          <div className="w-4 h-4 rounded-full border-2 border-white/20 border-t-white animate-spin" />
        ...
        {receipt && (
          <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
            <div className="glass-panel rounded-3xl p-5 sm:p-7 max-w-sm w-full shadow-2xl text-center animate-in fade-in zoom-in-95 duration-200">
              <div className="w-14 h-14 rounded-full bg-emerald-50 border-2 border-emerald-100 text-[#36543D] flex items-center justify-center mx-auto mb-3 shadow-xs">
              ...
              <div className="glass-panel-subtle rounded-2xl p-4 mb-5 text-left space-y-2 text-xs">
                ...
                <div className="pt-2 border-t border-neutral-200 flex justify-between font-extrabold text-sm">
                  ...
                  <span className="font-sans font-bold text-[#36543D]">{receipt.newBalance} pts</span>
                </div>
              ...
              <button
                onClick={resetPOS}
                className="w-full py-3.5 rounded-2xl bg-[#36543D] hover:bg-[#2a4230] text-white text-sm font-bold transition-all shadow-md active:scale-98 cursor-pointer flex items-center justify-center gap-2 font-sans"
              >
      ...
      <footer className="border-t border-neutral-200 bg-white py-2.5 px-4 text-center text-[11px] text-neutral-400 font-sans">
        {config.storeName || "ciao ciao"} POS Terminal • Real-Time Sync with Customer Pass
      </footer>
    </div>
  );
}
