...
const CUSTOMER_CACHE_KEY = "ciao_customer_cached";
...
export default function CustomerPage() {
  ...
  // Auto-Dismissing 6-Second Top Notification Toast State
  ...
  const loadNotifications = async () => {
    try {
      ...
      if (data.success) {
        ...
        // Find latest unread notification and trigger 6-second top toast if not already shown
  ...
  const syncPushSubscription = async (
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (typeof window !== "undefined") {
          localStorage.setItem("ciao_push_subscribed", "true");
        }
        return true;
      }
      return false;
  ...
  // Auto-dismiss top notification toast after 6 seconds without marking as read
  useEffect(() => {
  ...
  if (!customer) {
    return (
      <div className="min-h-screen bg-[#FAFBFA]/80 backdrop-blur-md flex flex-col justify-between p-6">
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <header className="glass-panel border-x-0 border-t-0 rounded-none sticky top-0 z-20 px-4 py-3">
        <div className="max-w-md mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl bg-white border border-neutral-200 shadow-xs p-1 object-contain flex items-center justify-center overflow-hidden">
              <img src="/logo.png" alt={config.storeName || "ciao ciao"} className="w-full h-full object-contain" />
            </div>
            <div>
      ...
      <main className="max-w-md mx-auto w-full px-4 pt-4 pb-32 flex-1">
        {/* Auto-Dismissing 6-Second Top Notification Toast */}
        {showTopToast && activeToastNotification && !activeToastNotification.isRead && activeTab !== "notifications" && (
          <div
            className="mb-4 bg-[#36543D] text-white rounded-2xl p-3.5 shadow-lg border border-white/10 flex items-center justify-between gap-3 animate-in fade-in slide-in-from-top-2 duration-300 relative overflow-hidden"
          >
            {/* Clickable body to open and read notification */}
            <div
              onClick={() => {
            ...
            >
              ...
              <span className="text-[10px] font-bold px-2.5 py-1 rounded-lg bg-white/20 text-white shrink-0 font-sans hover:bg-white/30 transition-colors">
                View
              </span>
            </div>

            {/* Quick mini dismiss 'X' button - does NOT mark notification as read */}
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation();
              ...
              className="p-1.5 rounded-lg text-white/70 hover:text-white hover:bg-white/10 transition-colors cursor-pointer shrink-0 ml-1"
              title="Close notice"
          ...
          <div className="space-y-4">
            ...
            <div className="glass-panel rounded-3xl p-6 shadow-xl relative overflow-hidden">
              ...
              <div className="flex items-start justify-between mb-5">
                ...
                <div className="w-10 h-10 rounded-xl bg-white border border-neutral-200 shadow-xs p-1 object-contain flex items-center justify-center overflow-hidden">
                  <img src="/logo.png" alt={config.storeName || "ciao ciao"} className="w-full h-full object-contain" />
                </div>
              </div>
                  ...
                  >
                    ...
                    <div className="relative h-48 sm:h-52 w-full bg-neutral-50 overflow-hidden">
                      ...
                      <div className="absolute top-3.5 start-3.5 px-3 py-1 rounded-full text-[11px] font-semibold bg-white/95 backdrop-blur-xs text-neutral-900 shadow-xs border border-neutral-200 font-sans">
                        {reward.category === "Pasta"
                          ? "Pasta"
                          : reward.category === "Pizza"
                          ? "Pizza"
                          : reward.category === "Drinks"
                          ? "Beverages"
                          : reward.category === "Desserts"
                          ? "Desserts"
                          : reward.category}
                      </div>
                    </div>
      ...
      <nav className="fixed bottom-[calc(1.25rem+env(safe-area-inset-bottom,0px))] left-0 right-0 z-50 px-4 pointer-events-none flex justify-center">
        <div className="w-full max-w-sm glass-nav rounded-full p-1.5 shadow-[0_16px_40px_-8px_rgba(66,110,73,0.18),0_4px_16px_rgba(0,0,0,0.08)] border border-white/90 pointer-events-auto flex items-center justify-between gap-1">
          <button
