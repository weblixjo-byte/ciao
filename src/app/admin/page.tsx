...
const i18n = {
  en: {
    ...
    analyticsSubtitle: "Real-time overview of loyalty performance, points flow, and member activity.",
    ...
    auditTrail: "Points & Loyalty Audit",
...
export default function AdminPage() {
  ...
  useEffect(() => {
    setLang("en");
    if (typeof window !== "undefined") {
      localStorage.setItem("ciao_admin_lang", "en");
    }
  }, []);
  ...
  const handleCreateReward = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setNewReward({ title: "", description: "", pointsRequired: 100, category: "Drinks", imageUrl: "", claimCode: "", stock: 999 });
        await loadRewards();
      } else {
  ...
  if (!admin) {
    return (
      ...
      <div
        dir="ltr"
        className="min-h-screen bg-[#FAFBFA] flex flex-col justify-between p-4 sm:p-6 transition-all"
      >
        <div className="max-w-sm w-full mx-auto my-auto py-4">
          <div className="text-center mb-8">
            <div className="w-16 h-16 rounded-2xl bg-white border border-neutral-200 shadow-xs p-1 object-contain flex items-center justify-center mx-auto mb-4 overflow-hidden">
  ...
  return (
    <div
      dir={lang === "ar" ? "rtl" : "ltr"}
      className="min-h-screen bg-[#FAFBFA] flex flex-col lg:flex-row transition-all text-neutral-900"
    >
      {/* MOBILE & IPAD PORTRAIT TOP HEADER (< lg: 1024px) */}
      <div className="lg:hidden bg-white/95 backdrop-blur-md border-b border-[#dce5dd] sticky top-0 z-30">
        <div className="px-4 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="w-10 h-10 rounded-xl bg-white border border-neutral-200 shadow-xs p-1 object-contain flex items-center justify-center shrink-0 overflow-hidden">
              <img src="/logo.png" alt={config.storeName || "ciao ciao"} className="w-full h-full object-contain" />
            </div>
        ...
        <div className="px-3 py-2 border-t border-[#dce5dd]/60 overflow-x-auto flex items-center gap-1.5 scrollbar-none">
          {navTabs.map((tab) => {
            const Icon = tab.icon;
      ...
      <aside className="hidden lg:flex w-64 xl:w-72 bg-white/70 backdrop-blur-2xl border-e border-white/60 shadow-xs flex-col justify-between p-5 min-h-screen shrink-0 sticky top-0 h-screen">
        <div>
          {/* Brand header */}
          <div className="flex items-center gap-3 px-2 py-3 mb-5 border-b border-[#dce5dd]/60">
            <div className="w-11 h-11 rounded-xl bg-white border border-neutral-200 shadow-xs p-1 object-contain flex items-center justify-center flex-shrink-0 overflow-hidden">
              <img src="/logo.png" alt={config.storeName || "ciao ciao"} className="w-full h-full object-contain" />
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="glass-panel rounded-3xl p-6 sm:p-7 shadow-sm">
                ...
                <div className="divide-y divide-[#dce5dd]/40">
                ...
                {rewardsList.map((reward) => (
                  ...
                  >
                    <div>
                      ...
                      <div className="relative h-44 sm:h-48 w-full bg-[#FAFBFA] overflow-hidden border-b border-[#dce5dd]/60">
                        {reward.imageUrl ? (
                          <img
                            src={reward.imageUrl}
                            alt={reward.title}
                        ...
                        <div className="absolute top-3 start-3 flex items-center gap-1.5">
                          <span className="px-2.5 py-1 rounded-full bg-white/95 backdrop-blur-xs text-neutral-900 border border-neutral-200 text-[10px] font-semibold shadow-2xs">
                            {reward.category}
                          </span>
                          <span className="px-2.5 py-1 rounded-full bg-neutral-900/90 backdrop-blur-xs text-[#F4EECF] border border-neutral-700 text-[10px] font-mono font-bold shadow-2xs">
      ...
      {showAddRewardModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
          ...
          >
            ...
            {createRewardError && (
              <div className="p-3.5 rounded-2xl bg-emerald-50/80 border border-emerald-200 text-xs text-emerald-800 flex items-center gap-2 mb-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewReward({
                        ...newReward,
                        claimCode: e.target.value.replace(/\D/g, "").slice(0, 2),
                      })
                    }
                    placeholder="Auto (e.g. 25)"
                    className="glass-input w-full font-mono font-bold text-center"
                  />
                </div>
              </div>
